...
def search_flights(origin: str, destination: str) -> str:
    ...
    # - Nếu tìm thấy → format danh sách chuyến bay dễ đọc
    # - Nếu không → thử tra ngược (destination, origin)
    # - Nếu vẫn không có → trả về thông báo không tìm thấy
    ...
    for f in flights:
        result.append(
            f"- {f['airline']} ({f['class']}): {f['departure']} -> {f['arrival']}. Giá: {f['price']:,} VNĐ"
        )
    return "\n".join(result)
    #pass
...
def search_hotels(city: str, max_price_per_night: int = 9999999) -> str:
    ...
    if not hotels:
        return f"Rất tiếc, TravelBuddy chưa có dữ liệu khách sạn tại {city}."
    ...
    result = [f"Khách sạn tại {city} (Ưu tiên đánh giá cao):"]
    for h in filtered_hotels:
        result.append(
            f"- {h['name']} ({h['stars']} sao): {h['price_per_night']:,} VNĐ/đêm. Khu vực: {h['area']}. Đánh giá: {h['rating']}/5"
        )
    return "\n".join(result)
    #pass
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    - expenses: chuỗi mô tả các khoản chi, format 'tên:số_tiền'
      (VD: 've_may_bay:890000,khach_san:650000')
    ...
    try:
        ...
        for item in expense_items:
            ...
            detail_lines.append(f"- {name.replace('_', ' ').capitalize()}: {amount:,} VNĐ")

        remaining = total_budget - total_expense

        output = ["--- BẢNG CHI TIẾT NGÂN SÁCH ---"]
        output.append(f"Tổng ngân sách: {total_budget:,} VNĐ")
        output.extend(detail_lines)
        output.append(f"Tổng chi phí: {total_expense:,} VNĐ")
        output.append("-" * 30)

        if remaining >= 0:
            output.append(f"Số dư còn lại: {remaining:,} VNĐ (Trong tầm kiểm soát!)")
        else:
            output.append(f"CẢNH BÁO: Vượt ngân sách {abs(remaining):,} VNĐ!")

        return "\n".join(output)
    except Exception as e:
        return "Lỗi định dạng expenses. Vui lòng sử dụng format 'tên:số_tiền,tên:số_tiền'."
    #pass
